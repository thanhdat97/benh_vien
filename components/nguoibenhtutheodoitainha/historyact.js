...
export default class app extends Component {
    ...
    componentDidMount() {
        ...
        AsyncStorage.getItem('DATA_LOGIN', (err, result) => {
            if (result != null) {
                ...
                fetch('http://27.72.76.115:8181/api/tuong-tac/get-all-testact/' + JSON_DATA.maBn)
                    .then((response) => response.json())
                    .then((responseData) => {
                        responseData.forEach(element => {
                            tableData.push([dateformat(element.ngay, 'dd-mm-yyyy HH:MM'), element.testAct])
                        });
    ...
    XL_Chon() {
        this.props.navigation.navigate("Man_hinh_bieudoact")
    }
    XL_Chon7() {
